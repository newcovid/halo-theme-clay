...
窗口连续，但**只在 CJK 区段之内连续**。`unicode-range` 描述的是「这一片里有什么」，
不是「这一片负责什么」：命中范围而字体里没有字形时，浏览器仍然会先把整片下下来，
发现没有、再回落到下一个字体族——白下一次。所以每个窗口都逐块裁进 `BLOCKS`。
早期版本把窗口写成一个跨块的大区间，最后两片顺带认领了 23952 个非 CJK 码位，
其中包括**私用区**（图标字体的码位都在这里）和**全部谚文音节**；
页面上只要出现一个图标字或一句韩文，就会凭空拉一片 52.9 KB 的中文生僻字。
